...
public class SpeakerAttributionFixService
{
    ...
    public SpeakerAttributionFixService(ILogger<SpeakerAttributionFixService> logger)
    {
        _logger = logger;
    }
    ...
    // Enhanced pattern arrays for better detection
    ...
    private void AnalyzeTranscriptForResultStats(string transcript, SpeakerAttributionResult result)
    {
        ...
        foreach (string line in lines)
        {
            ...
            MatchCollection laughterMatches = LaughterRegex.Matches(text);
            ...
            MatchCollection mildProfanityMatches = MildProfanityRegex.Matches(text);
            MatchCollection strongProfanityMatches = StrongProfanityRegex.Matches(text);
            ...
            MatchCollection pejorativeMatches = PejorativeRegex.Matches(text);
    ...
    public async Task<SpeakerAttributionResult> FixSpeakerAttribution(string sessionPath, Dictionary<int, string> micAssignments)
    {
        ...
        try
        {
            ...
            foreach (WordCountUtterance masterUtterance in masterData.result.transcription.utterances)
            {
                ...
                (int? matchedMic, double matchScore) = FindBestMatch(masterUtterance, micUtterances);
