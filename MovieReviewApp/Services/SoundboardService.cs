    ...
    public class SoundboardService
    {
        ...
        public async Task<bool> DeleteSoundClipAsync(string id)
        {
            try
            {
                ...
                var soundClip = await _mongoDbService.GetByIdAsync<SoundClip>(soundId);
                if (soundClip == null)
                {
                    return false;
                }
        ...
        public async Task<List<SoundClip>> GetSoundClipsForPersonAsync(string personId)
        {
            try
            {
                return await _mongoDbService.FindAsync<SoundClip>(s => s.PersonId == personId && s.IsActive);
            }
            catch (Exception ex)
            {
        ...
        public async Task<Dictionary<string, int>> GetSoundClipCountsByPersonAsync()
        {
            try
            {
                var allClips = await _mongoDbService.FindAsync<SoundClip>(s => s.IsActive);
                return allClips
